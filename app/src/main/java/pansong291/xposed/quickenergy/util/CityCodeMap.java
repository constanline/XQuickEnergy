...
public class CityCodeMap {
    private static final String TAG = CityCodeMap.class.getCanonicalName();
    ...
    public static void saveIdMap() {
        if (hasChanged) {
            ...
            hasChanged = !FileUtils.write2File(sb.toString(), FileUtils.getCityCodeMapFile());
        }
    }
    ...
    public static Map<String, String> getIdMap() {
        if (idMap == null || shouldReload) {
            shouldReload = false;
            idMap = new TreeMap<>();
            String str = FileUtils.readFromFile(FileUtils.getCityCodeMapFile());
